...
Running `bun run abstract-document/src/main.ts` (or `bun run abstract-document` from the repo root) produces:
...
| Build/test                         | Maven module (`pom.xml`), JUnit 5                                                                   | Bun (`package.json`), `bun:test`                                                                              | `bun test` is Jest-API-compatible                                                                                                                            |
